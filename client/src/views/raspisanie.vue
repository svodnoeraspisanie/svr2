  ...
  methods: {
    async zagruzkaraspisaniya (mesto){
      ...
      for (let sob=0; sob < vcalendar.jCal[2].length;sob++){
          ...
          novsob.start=start.getFullYear()+'-'+(start.getMonth()+1)+'-'+(start.getDate()+1)+' '+start.getHours()+':'+start.getMinutes();
          let end = new Date(vcalendar.jCal[2][sob][1][1][3]);
          novsob.end=end.getFullYear()+'-'+(end.getMonth()+1)+'-'+(end.getDate()+1)+' '+end.getHours()+':'+end.getMinutes();
